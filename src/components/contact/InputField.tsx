...
export const InputField: React.FC<InputFieldProps> = ({ label, isTextArea, className, ...props }) => {
    ...
    return (
        <div className="relative mt-2">
            ...
            <label
                htmlFor={inputId}
                className="absolute left-0 -top-3.5 text-xs text-slate-500 transition-all peer-placeholder-shown:top-3 peer-placeholder-shown:text-base peer-placeholder-shown:text-slate-400 peer-focus:-top-3.5 peer-focus:text-xs peer-focus:text-brand-primary font-medium tracking-wide cursor-text"
            >
                {label}
            </label>
        </div>
